...
def profile_selector(driver):
    ...
    driver.find_element(By.XPATH, '//*[@id="CPHPage_ddState"]/option[22]').click()
    time.sleep(5)
    ...
    for i in range(len(dist_options)):
        dist_options = driver.find_elements(By.XPATH, '//*[@id="CPHPage_ddDistrict"]/option')[1:]
        dist_name = sanitize_filename(dist_options[23].text)
        dist_options[23].click()

        time.sleep(5)

        block_options = driver.find_elements(By.XPATH, '//*[@id="CPHPage_ddBlock"]/option')[1:]
        for j in range(len(block_options)):
            block_options = driver.find_elements(By.XPATH, '//*[@id="CPHPage_ddBlock"]/option')[10:11]
            ...
            for d in range(len(panchayat_options)):
                panchayat_options = driver.find_elements(By.XPATH, '//*[@id="CPHPage_ddPanchayat"]/option')[47:]
                ...
                for v in range(len(village_options)):
                    ...
                    custom_dir = os.path.join(r"D:\jjm\Maharashtra", dir)
                    ...
                    # Go back to re-select the district, block, panchayat, etc.
                    # driver.back()
                    # time.sleep(5)
